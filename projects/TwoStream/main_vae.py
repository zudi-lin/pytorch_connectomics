...
from twostream.trainer import TrainerVAE
from twostream.config import add_twostream_config


def main():
    args = get_args()
    cfg = load_cfg(args, freeze=False, add_cfg_func=add_twostream_config)
    ...
    trainer = TrainerVAE(cfg, device=device, mode=mode, rank=args.local_rank, 
