...
class SwinUNETR(nn.Module):
    """
    Swin UNETR model, adapted from: Hatamizadeh et al., <https://arxiv.org/abs/2201.01266>.
    ...
    Args:
        ...
        downsample (str): module used for downsampling, available options are ```'mergingv2'```, ```'merging'``` and a
            user-specified ```nn.Module``` following the API defined in :py:class:```monai.networks.nets.PatchMerging```.
            The default is currently ```'merging'``` (the original version defined in v0.9.0).
        use_v2 (bool): use swinunetr_v2, which adds a residual convolution block at the beggining of each swin stage.
    """
...
class PatchMergingV2(nn.Module):
    ...
    def __init__(self, dim: int, norm_layer: type[LayerNorm] = nn.LayerNorm, spatial_dims: int = 3, merge_z: bool = True) -> None:
        ...
        Args:
            ...
            merge_z: whether to merge z axis (False for anisotropic data).
        """
...
class BasicLayer(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            use_checkpoint: use gradient checkpointing for reduced memory usage.
            is_isotropic: whether each stage is isotropic. Default: False
        """
