...
class Visualizer:
    ...
    def _visualize_grid(
        ...
        self._log_visualization(
            volume, label_viz, output_viz, writer, iteration, prefix
        )
    ...
    def _log_visualization(
        ...
        if output.shape[1] == 1:
            self._log_single_channel_viz(
                volume_rgb, label, output, writer, iteration, prefix
            )
        else:
    ...
    def _log_single_channel_viz(
        ...
        # Stack: [input, prediction (grayscale), ground_truth (grayscale/multi-color)]
