...
def blend_gaussian(
    sz: Union[Tuple[int], List[int]], sigma: float = 0.2, mu: float = 0.0
) -> np.ndarray:
    """
    ...
    zz, yy, xx = np.meshgrid(
        np.linspace(-1, 1, sz[0], dtype=np.float32),
...
def blend_bump(sz: Union[Tuple[int], List[int]], t: float = 1.5) -> np.ndarray:
    """
    ...
    """
    zz, yy, xx = np.meshgrid(
        np.linspace(0, 1, sz[0] + 2, dtype=np.float32)[1:-1],
