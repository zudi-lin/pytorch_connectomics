...
from connectomics.model.backbone.repvgg import RepVGGBlock2D
...
class TestModelBlock(unittest.TestCase):
    ...
    def test_repvgg_block_2d(self):
        b, h, w = 2, 32, 32
        c_in, c_out = 8, 8
        train_block = RepVGGBlock2D(c_in, c_out, deploy=False)
        train_block.eval()
        kernel, bias = train_block.repvgg_convert()
        deploy_block = RepVGGBlock2D(c_in, c_out, deploy=True)
