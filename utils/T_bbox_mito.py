...
# Parameters
#-------------------------------
D0 = '/n/coxfs01/zudilin/research/synapseNet/data/mito/Jwr/'
mask_path = D0 + 'mito100'
#bbox_path = D0 + 'bbox'
...
def get_dataset_label(path, x0p, x1p, y0p, y1p, z0p, z1p):
    ...
    sz = (z1p-z0p, y1p-y0p, x1p-x0p)
    result = np.zeros(sz, dtype=np.uint8)
    # floor
    zf = z0p // volume_size[0]
    yf = y0p // volume_size[1]
    xf = x0p // volume_size[2]
    # ceiling
    zc = min(z1p // volume_size[0] + 1, (z1-z0) // volume_size[0])
    yc = min(y1p // volume_size[1] + 1, (y1-y0) // volume_size[1])
    xc = min(x1p // volume_size[2] + 1, (x1-x0) // volume_size[2])
    ...
    for z in range(zf, zc):
        for y in range(yf, yc):
            for x in range(xf, xc):
                zp0 = z * volume_size[0]
                zp1 = (z+1) * volume_size[0]
                yp0 = y * volume_size[1]
                yp1 = (y+1) * volume_size[1]
                xp0 = x * volume_size[2]
                xp1 = (x+1) * volume_size[2]
...
if __name__== "__main__":
    #bbox = [8,12,0,7,744,798]
    #bbox = [73,80,146,175,297,344]
    bbox = [8,17,360,393,816,858]
    bbox = [8,13,710,731,500,526]
    bbox = [1779,1781,7446,7464,11012,11035]
    image, label = process_bbox(bbox, gt=True)    
