...
def mask2vector(masks, scale_factor=(4,8,8)):
    # masks: a numpy.array of size (N,C,Z,Y,X), e.g. (N,3,8,160,160)
    mask_shape = masks.shape
    print('Input info: ', mask_shape, masks.dtype)
    down_shape = np.array(mask_shape)
    down_shape[2:] = down_shape[2:] // np.array(scale_factor)

    output_shape = (down_shape[0], down_shape[1]*down_shape[2]*down_shape[3]*down_shape[4])
    print('Downsample shape: ', down_shape)
    print('Output shape: ', output_shape)

    # Invert mask back to pre-sigmoid
    masks = masks.astype(np.float32)
    masks = masks / 255.0
    masks = np.clip(masks, 0.01, 0.99)
    pre_activation = np.log(masks/(1.0-masks))

    resized = resize(pre_activation, down_shape, order=1, mode='reflect', 
                     cval=0, clip=True, preserve_range=True, anti_aliasing=False)
    vectors = np.reshape(resized, output_shape)
    return vectors

class Mask2VectorDataset(torch.utils.data.Dataset):
    def __init__(self,
                 bbox_text,
                 g_text=None,
                 b_text=None):
    ...
    def __getitem__(self, index):
        ...
        image, pseudo_label = process_bbox(input_bbox, gt=True, use_gt=True)
        return image, pseudo_label, index
...
def collate_fn_mask(batch):
    ...
    image, pseudo_label, index = zip(*batch)
    return image, pseudo_label, index
...
if __name__== "__main__":
    ...
    dataset = Mask2VectorDataset(bbox_text, g_text, b_text)  
    loader = torch.utils.data.DataLoader(
            dataset, batch_size=128, shuffle=False, collate_fn = collate_fn_mask,
            num_workers=8, pin_memory=True)

    MASK = []
    for iteration, (image, pseudo_label, index) in enumerate(loader):
        if iteration == 0: print(len(pseudo_label), pseudo_label[0].shape, pseudo_label[0].dtype)
        label_list = [x for x in pseudo_label]
        MASK  = MASK + label_list
        print('%d\t%d\t%d' % (iteration, iteration*128, 28700))

    masks = np.stack(MASK, 0)
    print(masks.shape, masks.dtype)
    vectors = mask2vector(masks, scale_factor=(4,8,8))
  
    hk = h5py.File('synapse_vectors_parallel.h5','w')
    hk.create_dataset('main',data=vectors, compression='gzip')
    hk.close()